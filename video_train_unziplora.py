...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if video_paths:  # Video files mode
            ...
            for video_path in video_paths:
                ...
                if len(frames) == num_frames:
                    ...
                    print(f"Warning: {video_path} only has {len(frames)} frames, skipping (need {num_frames})")
        else:  # Frame folders mode
            video_folders = [d for d in Path(instance_data_root).iterdir() if d.is_dir()]
            print(f"Loading frames from {len(video_folders)} folders...")
            for folder in video_folders:
                ...
                if len(frame_paths) == num_frames:
                    ...
                    print(f"Warning: {folder} only has {len(frame_paths)} frames, skipping (need {num_frames})")
        
        if len(instance_videos) == 0:
            raise ValueError(f"No valid videos found in {instance_data_root}. Each video needs {num_frames} frames.")
        
        print(f"Loaded {len(instance_videos)} videos with {num_frames} frames each")
        
        if oneshot is True:
            selected_video = random.choice(instance_videos)
            instance_videos = [selected_video]
            print(f"One-shot mode: Selected video with {len(selected_video)} frames")
        
        self.custom_instance_prompts = None
        self.instance_videos = []
        
        # Repeat videos based on repeats parameter
        for video in instance_videos:
            self.instance_videos.extend(itertools.repeat(video, repeats))
        
        self.num_instance_videos = len(self.instance_videos)
        self.length = self.num_instance_videos
        
        # Load class images (content)
        if class_data_root is not None:
            ...
            self.length = max(self.num_class_images, self.num_instance_videos)
        else:
            self.class_data_root = None
        
        # Load class images 2 (style)
    ...
    def __getitem__(self, index):
        ...
        # Get video frames
        instance_video = self.instance_videos[index % self.num_instance_videos]
        ...
        if self.custom_instance_prompts:
            caption = self.custom_instance_prompts[index % self.num_instance_videos]
            if caption:
        ...
        # Class images (single images, not videos)
...
def collate_fn(examples, with_prior_preservation=False, with_vector_loss=False, class_data_root=None, class_data_root_2=None):
    # Collect video frames: each example["instance_images"] has shape [num_frames, C, H, W]
    pixel_values = [example["instance_images"] for example in examples]
    prompts = [example["instance_prompt"] for example in examples]
    
    # Stack: [batch, num_frames, C, H, W]
    ...
    if with_prior_preservation:
        if class_data_root is not None:
            # Class images are single images: [batch, C, H, W]
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                for key in model_inputs_key_list:
                    # Inside: for key in model_inputs_keylist:
                    pixel_values = pixel_values_dict[f"pixel_values_{key}"].to(dtype=vae.dtype)
                    prompts = prompts_dict[f"prompts_{key}"]

                    if len(pixel_values.shape) == 5:  # Video: [batch, num_frames, C, H, W]
                        batch_size_original = pixel_values.shape[0]
                        num_frames = pixel_values.shape[1]
                        
                        # Flatten: [batch*num_frames, C, H, W]
                        pixel_values = pixel_values.reshape(
                            batch_size_original * num_frames,
                            pixel_values.shape[2],
                            pixel_values.shape[3],
                            pixel_values.shape[4]
                        )
                    else:  # Single images: [batch, C, H, W]
                        batch_size_original = pixel_values.shape[0]
                        num_frames = 1
                    ...
                    # shape: [batch*num_frames, latent_C, latent_H, latent_W]
                    model_inputs[key] = model_input
                # After the loop, before adding noise:
                bsz = model_inputs["both"].shape[0]  # This is batch*num_frames for videos
                ...
                timesteps = torch.randint(
                    ...
                    (bsz,),
                    device=model_inputs["both"].device
                )
                timesteps = timesteps.long()

                # Add noise
                noisy_model_input = noise_scheduler.add_noise(model_inputs["both"], noise, timesteps)

                # Predict the noise residual
                if not args.train_text_encoder:
                    unet_added_conditions = {
                        "time_ids": add_time_ids.repeat(bsz, 1),  # Match flattened batch
                        "text_embeds": unet_add_text_embeds.repeat(bsz, 1),
                    }
                    
                    prompt_embeds_input = prompt_embeds.repeat(bsz, 1, 1)
                    prompt_embeds_style_forward_input = prompt_embeds_style_forward.repeat(bsz, 1, 1)
                    prompt_embeds_content_forward_input = prompt_embeds_content_forward.repeat(bsz, 1, 1)

                    # Forward pass (shape: [batch*num_frames, ...])
                    model_pred = unet(
                        noisy_model_input,
                        timesteps,
                        prompt_embeds_input,
