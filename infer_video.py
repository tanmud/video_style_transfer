        ...
        if len(args.validation_prompt) != 0 and args.validation_prompt != "":
            ...
            if args.with_unziplora:
                pipeline.unet = insert_unziplora_to_unet(
                    pipeline.unet,
                    f"{args.output_dir}/content",
                    f"{args.output_dir}/style",
                    weight_content_path=f"{args.output_dir}/merger_content.pth",
                    weight_style_path=f"{args.output_dir}/merger_style.pth",
                    rank=args.rank,
                )
            else:
                pipeline.unet = insert_unziplora_to_unet(
